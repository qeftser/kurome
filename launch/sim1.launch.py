...
def generate_launch_description():
    ...
    default_rviz_config_path = os.path.join(pkg_share, 'config/sim1_config.rviz')
    default_config_path = os.path.join(pkg_share, 'config/sim0_config.yaml')
    ...
    yoriko_node = launch_ros.actions.Node(
        ...
        name='yoriko',
        parameters=[LaunchConfiguration('config')],
        #prefix="valgrind -s --track-origins=yes"
    )
    ...
    return launch.LaunchDescription([
        ...
        launch.actions.ExecuteProcess(cmd=['gazebo', '--verbose', '-s', 'libgazebo_ros_init.so', '-s', 'libgazebo_ros_factory.so', os.path.join(pkg_share,'config/trash_cans.sdf')], output='screen'),
